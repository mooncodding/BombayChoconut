<template>
  <v-app>
    <div class="content-wrapper">
      <div v-if="is('Super Admin') || can('system_users')">
        <!-- Content Header (Page header) -->
        <div class="content-header">
          <div class="container-fluid">
            <div class="row">
              <div class="col-6">
                <h1 class="m-0 text-dark">{{ $t("message.SYSTEM_USERS") }}</h1>
              </div>
              <!-- /.col -->
              <div class="col-6">
                <button class="btn btn-success float-right" @click="addUser" v-if="(is('Super Admin') || can('create_system_user'))">
              ...
              <div class="col-12">
                <div class="card">
                  ...
                  <div class="card-body table-responsive p-0">
                    <v-data-table
                      :headers="headers"
                      :items="systemUsers.data ? systemUsers.data : systemUsers"
                      :server-items-length="systemUsers.total"
                      class="elevation-1"
                      :loading="loading"
                    ...
                    >
                      <template v-slot:item.actions="{ item }">
                        ...
                        <v-icon
                          ...
                          small
                          @click="editUser(item)"
                          v-if="(is('Super Admin') || can('edit_system_user'))"
                        >
                          mdi-pencil
                        </v-icon>
                        ...
                        <v-icon
                          ...
                          v-if="(is('Super Admin') || can('delete_system_user'))"
                        >
                          mdi-delete
                        </v-icon>
                      </template>
        ...
        <!-- /.content -->
        <addEditSystemUserModal></addEditSystemUserModal>
        <viewSystemUserModal :systemUserData="systemUserInfo"></viewSystemUserModal>
      </div>
...
<script>
import addEditSystemUserModal from "./modals/addEditSystemUserModal.vue";
import viewSystemUserModal from "./modals/viewSystemUserModal.vue";
export default {
  data() {
    return {
      form: new form(),
  ...
  methods: {
    getResults(page = 1, rows = 10, sortBy = null, sortDesc = null) {
      if (this.is('Super Admin') || this.can('system_users')) {
        this.$Progress.start();
        this.loading = true;
        this.curpage = page;
    ...
    deleteUser(id) {
      if (this.is('Super Admin') || this.can('delete_system_user')) {
        swal
          .fire({
    ...
    addUser() {
      if (this.is('Super Admin') || this.can('create_system_user')) {
        $("#addEditSystemUserModal").modal("show");
      }else{
        toast.fire({
          icon: "error",
          title: this.$t("message.UNAUTHORIZED")
    ...
    editUser(systemUser) {
      if (this.is('Super Admin') || this.can('edit_system_user')) {
        this.systemUserInfo = systemUser;
        $("#addEditSystemUserModal").modal("show", systemUser);
      }else{
        toast.fire({
          icon: "error",
          title: this.$t("message.UNAUTHORIZED")
    ...
    viewUser(systemUser) {
      this.systemUserInfo = systemUser;
      $("#viewSystemUserModal").modal("show", systemUser);
    },
  },
  created() {
    Fire.$on("reloadSystemSystemUsers", () => {
